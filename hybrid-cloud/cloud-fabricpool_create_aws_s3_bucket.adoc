...
image:cloud-fabricpool_image10.png["一張圖表，顯示有效和無效的 FabricPool Aggregate 對儲存桶的附件配置"]
...
. 在雲端中選取S3可擴充儲存設備。
+
image:cloud-fabricpool_image11.png["AWS 主控台搜尋的快照，顯示 S3 Scalable Storage in the Cloud 結果已醒目提示"]

. 在S3首頁上、選取「Create Bucket（建立儲存區）」。
. 輸入DNS相容的名稱、然後選擇要建立儲存區的區域。
+
image:cloud-fabricpool_image12.png["AWS 建立儲存桶對話框的快照，包含儲存桶名稱、區域和「建立」按鈕"]
