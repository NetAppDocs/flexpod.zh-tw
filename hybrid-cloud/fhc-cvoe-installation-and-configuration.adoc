...
summary: 本節介紹如何安裝和設定Cloud Volumes ONTAP、 FlexPod、 ONTAP儲存和NetApp Console。 
---
= 安裝與組態
:hardbreaks:
:allow-uri-read: 
...
. 存取NetApp Console。
+
要存取控制台和其他雲端服務，您需要註冊。 https://cloudmanager.netapp.com/["NetApp Console"^] 。有關在控制台帳戶中設定工作區和使用者的信息，請參閱 https://docs.netapp.com/us-en/console-setup-admin/index.html["NetApp Console設定與管理"^]。您需要一個具有權限的帳戶，才能直接從控制台在您的雲端提供者部署控制台代理程式。若要取得所需的權限，請參閱 https://docs.netapp.com/us-en/cloud-manager-setup-admin/reference-permissions.html["NetApp Console的權限摘要"^]。

. 部署控制台代理程式。
+
在新增 Cloud Volume ONTAP系統之前，必須先部署控制台代理程式。如果您嘗試在沒有安裝控制台代理的情況下建立第一個Cloud Volumes ONTAP系統，控制台會提示您。若要從控制台在 AWS 中部署控制台代理，請參閱下列內容： https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-creating-connectors-aws.html["AWS 中的控制台代理安裝選項"^] 。
...
+
在這個解決方案中，我們在 AWS 上部署了一個單節點Cloud Volumes ONTAP系統。
...
有關部署內部部署 FlexPod 執行個體的資訊，請參閱 https://www.cisco.com/c/en/us/td/docs/unified_computing/ucs/UCS_CVDs/flexpod_xseries_vmware_7u2.html["本部署指南"^]。
...
== 將本機FlexPod儲存加入到NetApp Console

使用控制台完成以下步驟，將FlexPod儲存新增至系統中。

. 從導覽選單中，選擇“儲存”>“系統”。
. 在“系統”頁面上，按一下“新增系統”，然後選擇“本機部署”。
. 選取*內部部署ONTAP 的S--。單擊 * 下一步 * 。
. 在「叢集詳細資料」頁面上、輸入叢集管理IP位址和管理員使用者帳戶的密碼ONTAP 。然後按一下「*新增*」。
. 在「詳細資料與認證」頁面上、輸入工作環境的名稱與說明、然後按一下「*執行*」。
+
控制台發現ONTAP集群，並將其作為系統添加到「系統」頁面。
