...
image:sm-bcs-image39.png["顯示 VMware HA 與 vSphere Metro Storage Cluster 透過多站台架構跨 FlexPod 站台 A 與 B 的圖表"]
...
image:sm-bcs-image40.png["此示意圖顯示位於站台 A 和站台 B 的 Cisco UCS B200 M5 伺服器和 AFF A250 儲存設備，透過 Nexus 交換器和位於站台 C 的 ONTAP Mediator 連接。"]
...
image:sm-bcs-image41.png["圖中顯示了 vSwitch0、vDS0、iSCSI-A 和 iSCSI-B 交換器以及六個 vNIC 上行鏈路。"]
...
image:sm-bcs-image42.png["顯示站台 A 和站台 B 主機和虛擬機器的 VM/主機群組的快照（在 vSphere 中）"]

此外、下圖顯示使用「應該在群組中的主機上執行」原則、為站台A和站台B VM所建立的VM/主機規則、以便在其各自站台的主機上執行。

image:sm-bcs-image43.png["這是「編輯虛擬機器/主機規則」對話框的螢幕截圖，其中設定了站點 A 的虛擬機器在站點 A 的主機上執行。"]

image:sm-bcs-image44.png["這是網站 B 虛擬機器和主機的「編輯虛擬機器/主機規則」對話方塊的螢幕截圖，其中「應在群組中的主機上執行」群組原則適用於這些虛擬機器和主機。"]
...
image:sm-bcs-image45.png["vSphere HA 進階選項的快照，其中已設定四個心跳資料存放區和兩個隔離位址。"]

對於活動訊號資料存放區、請從叢集指定四個共用資料存放區、並自動補強、如下圖所示。

image:sm-bcs-image46.png["vSphere HA 心跳資料儲存設定快照，其中已選擇四個指定的資料儲存。"]

有關 VMware HA 叢集和 VMware vSphere Metro 儲存設備叢集的其他最佳實務和組態，請參閱 https://docs.vmware.com/en/VMware-vSphere/7.0/com.vmware.vsphere.avail.doc/GUID-5432CA24-14F1-44E3-87FB-61D937831CF6.html["建立及使用vSphere HA叢集"^]、 https://core.vmware.com/resource/vmware-vsphere-metro-storage-cluster-vmsc["VMware vSphere Metro儲存叢集（VMSC）"^] 和 https://docs.netapp.com/us-en/netapp-solutions-virtualization/vmware/vmw-vmsc-with-smas.html["NetApp ONTAP 搭配 SnapMirror 作用中 Sync（以前稱為 SM-BC）和 VMware vSphere Metro Storage Cluster (vMSC)"^]。
