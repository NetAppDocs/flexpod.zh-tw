...
+
image:express-c-series-aff220-deploy_image8.png["一張 Cisco 伺服器開機畫面的快照，提示使用者按 F8 鍵進行 CIMC 設定。"]
...
*** 無：[x]
+
image:express-c-series-aff220-deploy_image9.png["Cisco IMC 網路卡屬性的快照，其中包含 IPv4 設定，但未選擇網路卡備援。"]
...
*** 連接埠設定檔：保持清除狀態。
+
image:express-c-series-aff220-deploy_image10.png["Cisco IMC 組態公用程式的快照，顯示常用屬性和連接埠設定"]
...
. 按一下「設定開機順序」、然後按一下「確定」。
+
image:express-c-series-aff220-deploy_image11.png["已選擇「設定開機順序」的 Cisco Integrated Management Controller BIOS 設定快照。"]
...
. 按一下[儲存變更]，然後按一下[關閉]。
+
image:express-c-series-aff220-deploy_image12.png["已啟用 KVM-MAPPED-DVD、iSCSI-A 和 iSCSI-B 的進階開機順序組態快照"]
...
. 如果該值尚未停用、請將其設為停用。
+
image:express-c-series-aff220-deploy_image13.png["快照顯示 CIMC BIOS 設定，其中 PCIe Slot 1 和 Slot 2 的 Option ROM 已停用。"]
...
** 啟用PXE開機：檢查
+
image:express-c-series-aff220-deploy_image14.png["快照顯示 iSCSI-vNIC-A 設定，MTU 9000、VLAN 3439、主幹連線模式，以及已啟用 PXE 開機。"]
...
+
image:express-c-series-aff220-deploy_image15.png["快照顯示已選取 iSCSI-vNIC-A，並在 vNIC 索引標籤上展開 iSCSI 開機屬性。"]
...
+
image:express-c-series-aff220-deploy_image16.png["Cisco UCS iSCSI 開機屬性的快照，顯示啟動器名稱、IP 位址、子網路遮罩和閘道欄位。"]
...
+
image:express-c-series-aff220-deploy_image17.png["顯示具有主要和次要 NetApp 目標的 iSCSI vNIC 啟動器設定的快照"]
...
+
image:express-c-series-aff220-deploy_image18.png["Cisco VIC vNICs 索引標籤的螢幕擷取畫面，其中列出了 eth0、eth1 和兩個 iSCSI vNIC。"]

. 選取eth0、然後按一下「內容」。
. 將MTU設為9000。按一下儲存變更。
+
image:express-c-series-aff220-deploy_image19.png["Cisco VIC eth0 vNIC 屬性的快照，其中 MTU 欄位設定為 9000"]

. 對eth1重複步驟3和4、確認上行鏈路連接埠對eth1設定為「1」。
+
image:express-c-series-aff220-deploy_image20.png["快照顯示 vNIC 索引標籤，其中 eth0 和 eth1 的 MTU 設定為 9000，上行連結連接埠為 0 和 1。"]
