...
. 導航至 `+https://<<FQDN or IP of vCenter>>/vsphere-client/+`。
...
. 按一下「確定」。
+
image:express-c-series-c190-deploy_image45.png["啟用 DRS 和 vSphere HA 但停用 vSAN 的「新叢集」對話方塊快照"]
...
+
image:express-c-series-c190-deploy_image46.png["vSphere Client 新增主機精靈的快照，其中包含主機 FQDN 輸入欄位和「下一步」按鈕。"]
...
. 登入 `https://<vCenter IP>:5480`，輸入使用者名稱 root，然後輸入 root 密碼。
. 按一下「服務」、然後選取「VMware vSphere ESXi傾印收集器」。
. 啟動VMware vSphere ESXi傾印收集器服務。
+
image:express-c-series-c190-deploy_image47.png["裝置管理服務的快照，其中已選擇 VMware vSphere ESXi Dump Collector，並且「啟動」可用。"]
...
+
image:express-c-series-c190-deploy_image48.png["終端機顯示 ESXi coredump 組態指令和 netdump 伺服器驗證"]
