...
+
image:express-c-series-aff220-deploy_image41.png["VMware ESXi 主機頁面快照，其中「取得 vCenter Server」按鈕已醒目提示"]
...
. 選取「內嵌平台服務控制器」作為部署類型。
+
image:express-c-series-aff220-deploy_image42.png["這是 vCenter Server 應用裝置部署類型選擇的快照，其中已選擇嵌入式平台服務控制器。"]
...
+
image:express-c-series-aff220-deploy_image43.png["vCenter 伺服器應用裝置部署目標頁面的快照，包含 ESXi 主機、連接埠、root 使用者和密碼欄位。"]

. 輸入「VCSA」作為VM名稱、並輸入您要用於VCSA的根密碼、以設定應用裝置VM。
+
image:express-c-series-aff220-deploy_image44.png["「設定應用裝置虛擬機器」頁面快照，包含虛擬機器名稱和根目錄密碼欄位"]

. 選擇最適合您環境的部署規模。按一下「下一步」
+
image:express-c-series-aff220-deploy_image45.png["vCenter Server 部署大小頁面的快照，已選取「微型」和「預設」儲存設備。"]

. 選取infra_datastore_1資料存放區。按一下「下一步」
+
image:express-c-series-aff220-deploy_image46.png["vCenter 伺服器安裝程式選擇資料儲存頁面的快照，其中已選擇 infra_datastore_1。"]
...
+
image:express-c-series-aff220-deploy_image47.png["vCenter 伺服器應用裝置安裝程式設定網路設定的快照，其中已填入靜態 IPv4 欄位。"]
...
. 在「第2階段簡介」頁面上、按「下一步」。
+
image:express-c-series-aff220-deploy_image48.png["vCenter Server 應用裝置第 2 階段介紹頁面的快照，其中含有「下一步」按鈕"]
