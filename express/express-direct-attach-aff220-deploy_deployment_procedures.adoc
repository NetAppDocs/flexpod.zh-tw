...
請依照控制器的實體安裝步驟進行操作 https://docs.netapp.com/us-en/ontap-systems/a220/install-setup.html["VA220文件AFF"^]。
...
+
image:express-direct-attach-aff220-deploy_image7.png["建立 IPv4 位址區塊對話方塊的快照，其中包含 IP 位址、子網路、閘道和 DNS 欄位"]
...
+
image:express-direct-attach-aff220-deploy_image8.png["「新增 NTP 伺服器」對話方塊的螢幕截圖，其中 NTP 伺服器設定為 10.1.156.4"]

. 按一下新增NTP伺服器。
. 輸入「<switch-b-ntp-ip>'或<Nexus B-mgmt-ip>'、然後按一下「OK（確定）」。按一下確認上的「OK（確定）」。
+
image:express-direct-attach-aff220-deploy_image9.png["Cisco UCS Manager NTP 設定的快照，其中列出了兩個 NTP 伺服器，並選擇了 America/New_York 時區。"]
...
+
image:express-direct-attach-aff220-deploy_image10.png["快照顯示已設定上行鏈路和應用裝置儲存設備角色的 Fabric Interconnect A 乙太網路連接埠。"]
...
+
image:express-direct-attach-aff220-deploy_image11.png["快照顯示已設定網路和應用裝置儲存設備角色的 Fabric Interconnect B 乙太網路連接埠。"]
...
. 按一下「下一步」
+
image:express-direct-attach-aff220-deploy_image12.png["輸入了 ID 為 1、名稱為 vPC-13-Nexus 的「建立連接埠通道」對話方塊的快照"]
...
+
image:express-direct-attach-aff220-deploy_image13.jpeg["建立 VLAN 對話方塊的螢幕擷取畫面，其中輸入了 NFS-VLAN 和 VLAN ID 3170"]
...
. 按一下「確定」、然後再按一下「確定」以建立VLAN。
+
image:express-direct-attach-aff220-deploy_image14.png["一張快照，顯示 Cisco UCS Manager 中的 VLAN 表，其中包含 VLAN 名稱和 ID。"]
...
. 啟用LLDP的傳輸和接收功能。
+
image:express-direct-attach-aff220-deploy_image15.png["快照顯示啟用 CDP 和 LLDP 傳送與接收的 Enable_CDP 網路控制原則。"]
...
+
image:express-direct-attach-aff220-deploy_image16.png["啟用 CDP 並選擇儲存設備 VLAN 後，裝置介面 1/3 屬性的快照。"]
...
+
image:express-direct-attach-aff220-deploy_image17.png["已選擇 iSCSI-B、NFS 和原生 VLAN 的應用裝置介面 1/3 VLAN 設定快照"]
...
+
image:express-direct-attach-aff220-deploy_image18.png["Cisco UCS Manager QoS 系統類別設定的快照，其中盡力而為 MTU 設定為 9216"]
...
. 為兩個刀鋒封裝選取版本4.0(1b)。
+
image:express-direct-attach-aff220-deploy_image19.png["「修改套件版本」對話方塊的快照，其中已選擇 Blade 套件 4.0(1b)B"]
...
+
image:express-direct-attach-aff220-deploy_image20.png["建立 MAC 位址區塊對話框的快照，MAC 位址為 00:25:B5:32:0A:00，大小為 64"]
...
+
image:express-direct-attach-aff220-deploy_image21.png["建立 IQN 後綴塊對話框的快照，後綴為 ucs-host，起始值為 1，大小為 16"]
...
+
image:express-direct-attach-aff220-deploy_image22.png["啟用 CDP 和 LLDP 後，「建立網路控制原則」對話方塊的快照。"]
...
+
image:express-direct-attach-aff220-deploy_image23.png["建立電源控制原則對話方塊的快照，其中已選擇「No-Power-Cap」和「No Cap」。"]
...
+
image:express-direct-attach-aff220-deploy_image24.png["Cisco UCS Manager 建立 CPU/核心資格認證對話方塊的快照，顯示 Xeon 處理器架構和 CPU PID 欄位"]
...
. 將「一致的裝置命名」變更為「已啟用」。
+
image:express-direct-attach-aff220-deploy_image25.png["建立 BIOS 原則對話框的快照，其中已啟用 VM 主機名稱和一致性裝置命名。"]
...
** 處理器C7報告：已停用
+
image:express-direct-attach-aff220-deploy_image26.png["建立 BIOS 原則處理器標籤的快照，其中處理器 C 狀態和 C1E 已停用。"]
...
** DRAM時鐘節流：效能
+
image:express-direct-attach-aff220-deploy_image27.png["快照顯示 BIOS 處理器選項，其中「能耗效能」和「DRAM 時脈節流」均設定為「效能」。"]
...
** 低電壓DDR模式：效能模式
+
image:express-direct-attach-aff220-deploy_image28.png["建立 BIOS Policy RAS Memory 頁面的快照，其中 LV DDR Mode 設定為 Performance Mode"]
...
+
image:express-direct-attach-aff220-deploy_image29.png["Cisco UCS Manager 預設維護原則的快照，其中已選擇「使用者確認」和「下次開機時」選項。"]
...
. 按一下「確定」。
+
image:express-direct-attach-aff220-deploy_image30.png["Cisco UCS Manager 中 Fabric A 上 vNIC_Template_A 的 vNIC 範本屬性畫面截圖"]
...
. 按一下「確定」。
+
image:express-direct-attach-aff220-deploy_image31.png["Cisco UCS Manager vNIC 範本 B 設定的快照，其中選擇了 Fabric B、MTU 9000 和 MAC 池 B"]
...
. 按一下「確定」。
+
image:express-direct-attach-aff220-deploy_image32.png["已選擇 Fabric A、MTU 9000 和 MAC Pool A56/64 的 Site_01_iSCSI-A vNIC 範本的螢幕擷取畫面"]
...
. 按一下「確定」。
+
image:express-direct-attach-aff220-deploy_image33.png["Cisco UCS Manager vNIC 範本 Site_01_iSCSI-B 設定的快照，適用於 MTU 為 9000 的 Fabric B"]
...
. 按一下「確定」、將此vNIC新增至原則。
+
image:express-direct-attach-aff220-deploy_image34.png["快照顯示使用 vNIC_Template_A 和 VMware 介面卡原則修改 Site-01-vNIC-A 的 vNIC 對話框"]
...
+
image:express-direct-attach-aff220-deploy_image35.png["Site-01-iSCSI-A 的「修改 iSCSI vNIC」對話方塊快照，其中包含 VLAN 和 MAC 位址設定"]
...
. 按一下儲存變更。
+
image:express-direct-attach-aff220-deploy_image36.png["一張顯示 Site-01-iSCSIBoot LAN 連線原則的快照，其中列出兩個 iSCSI vNIC。"]
...
+
此 VMware ESXi 6.7U1 ISO 可從 https://support.broadcom.com/group/ecx/downloads["Broadcom 支援入口網站"^]下載。
...
+
image:express-direct-attach-aff220-deploy_image37.png["建立 vMedia 裝載對話方塊的快照，其中已設定 HTTP ESXi 安裝程式 ISO 遠端檔案和路徑"]
...
. 按一下「確定」以建立原則。
+
image:express-direct-attach-aff220-deploy_image38.png["Cisco UCS Manager 開機原則 Site-01-Fabric-A 的快照，其中已設定兩個 iSCSI 開機項目。"]
...
+
image:express-direct-attach-aff220-deploy_image39.png["建立 Service Profile Template 精靈的快照，包含名稱、更新範本和 UUID 資源池欄位。"]
...
+
image:express-direct-attach-aff220-deploy_image40.png["建立 Service Profile Template 連網設定的快照，其中已選擇 iSCSI-Boot 連線和 IQN_Pool。"]
...
+
image:express-direct-attach-aff220-deploy_image41.png["建立 Service Profile Template 伺服器開機順序頁面的快照，其中已選擇 Boot-Fabric-A。"]
...
+
image:express-direct-attach-aff220-deploy_image42.png["iscsi show 命令輸出的快照，其中列出 Infra-SVM 目標 IQN、別名和運作狀態"]

. 在「IPV4位址」欄位中輸入「iscsit_lif_02A」的IP位址。
+
image:express-direct-attach-aff220-deploy_image43.png["建立 iSCSI 靜態目標對話方塊的快照，其中輸入了 IPv4 位址 192.168.10.62"]
...
+
image:express-direct-attach-aff220-deploy_image44.png["建立 iSCSI 靜態目標對話方塊的快照，IPv4 位址為 192.168.10.61，LUN ID 為 0。"]

. 按一下「確定」以新增iSCSI靜態目標。
+
image:express-direct-attach-aff220-deploy_image45.png["已選擇 iSCSI 靜態目標介面的「設定 iSCSI 開機參數」對話方塊快照。"]
...
+
image:express-direct-attach-aff220-deploy_image46.png["建立 iSCSI 靜態目標對話框的快照，其中 IPv4 位址設定為 192.168.20.62"]
...
+
image:express-direct-attach-aff220-deploy_image47.png["建立 iSCSI 靜態目標對話方塊的快照，IPv4 位址為 192.168.20.61，LUN ID 為 0"]

. 按一下「確定」以新增iSCSI靜態目標。
+
image:express-direct-attach-aff220-deploy_image48.png["快照顯示在「設定 iSCSI 開機參數」中選擇了兩個 iSCSI 靜態目標介面。"]
...
. 將維護原則變更為預設值。
+
image:express-direct-attach-aff220-deploy_image49.png["建立 Service Profile Template 維護原則步驟的快照，其中已選擇預設原則。"]
...
+
image:express-direct-attach-aff220-deploy_image50.png["建立 Service Profile Template 伺服器指派的快照，已選取 Infra-Pool、Down 電源狀態及預設韌體套件。"]
...
+
image:express-direct-attach-aff220-deploy_image51.png["建立 Service Profile Template Operational Policies 頁面的快照，其中已選擇 VM-Host BIOS 和 No-Power-Cap。"]
...
.. 按一下「確定」以建立服務設定檔。
+
image:express-direct-attach-aff220-deploy_image52.png["「從範本建立服務設定檔」對話方塊的快照，前綴為 Site-01-Infra-0，編號為 2，根目錄組織"]
...
. 登入 Broadcom 支援入口網站下載網站： https://support.broadcom.com/group/ecx/downloads["https://support.broadcom.com/group/ecx/downloads"^]。
. 您需要 Broadcom 使用者 ID 和密碼。請尋找 VMware vSphere Hypervisor (ESXi) 自訂 ISO 映像。
. 下載適用於 ESXi 6.7U1 安裝光碟的 Cisco 自訂映像。
...
. 按Enter。
+
image:express-direct-attach-aff220-deploy_image53.png["ESXi 網路介面卡畫面的螢幕截圖，其中已選取並連接 vmnic0 和 vmnic1。"]
...
+
image:express-direct-attach-aff220-deploy_image54.png["快照顯示 vSwitch0 的詳細資訊和拓撲，包括 VMkernel 介面卡和兩個實體介面卡。"]

. 選取[VMkernel NIC（VMkernel NIC）]索引標籤以確認已設定的虛擬介面卡。列出的介面卡應類似下列範例：
+
image:express-direct-attach-aff220-deploy_image55.png["ESXi VMkernel NIC 索引標籤的快照，其中列出了五個 VMkernel 介面卡及其連接埠群組"]
...
+
image:express-direct-attach-aff220-deploy_image56.png["ESXi 儲存設備介面卡索引標籤的快照，其中已選取 iSCSI 軟體介面卡 vmhba64。"]
...
. 按一下儲存組態。
+
image:express-direct-attach-aff220-deploy_image57.png["「Configure iSCSI」對話方塊的快照，顯示了連接埠 3260 上的四個動態和靜態 iSCSI 目標"]
...
+
image:express-direct-attach-aff220-deploy_image58.png["這是「新資料儲存」精靈的快照，其中已選擇「掛載 NFS 資料儲存」作為建立類型。"]
...
.. 按一下「下一步」
+
image:express-direct-attach-aff220-deploy_image59.png["提供 NFS 裝載詳細資訊頁面的快照，其中包含 infra_datastore_1 和 NFS 伺服器 192.168.104.3"]
...
. 按一下「完成」。資料存放區現在應該會出現在資料存放區清單中。
+
image:express-direct-attach-aff220-deploy_image60.jpeg["ESXi 儲存資料儲存清單的快照，顯示 infra_datastore_2 已裝載為 NFS 資料儲存。"]
...
+
image:express-direct-attach-aff220-deploy_image61.png["啟用 NTP 並輸入兩個 NTP 伺服器位址的「編輯時間組態」對話方塊快照"]
...
+
image:express-direct-attach-aff220-deploy_image62.png["VMware ESXi 管理系統交換頁面的快照，顯示交換設定和編輯設定"]

. 按一下「編輯設定」。從資料存放區選項中選取「infra_swap」。
+
image:express-direct-attach-aff220-deploy_image63.png["已選取 infra_swap 資料儲存的「編輯交換組態」對話方塊快照，其中包含「儲存」和「取消」按鈕"]
...
+
image:express-direct-attach-aff220-deploy_image64.png["ESXi 主機導覽器快照，其中「取得 vCenter 伺服器」按鈕已醒目提示"]
...
. 選取「內嵌平台服務控制器」作為部署類型。
+
image:express-direct-attach-aff220-deploy_image65.png["vCenter 伺服器設備安裝程式選擇部署類型頁面的快照，其中已選擇嵌入式平台服務控制器。"]
...
+
image:express-direct-attach-aff220-deploy_image66.png["已輸入 ESXi 主機憑證的 vCenter Server Appliance 安裝程式 Appliance 部署目標頁面的快照"]

. 輸入VCSA做為VM名稱、並輸入要用於VCSA的root密碼、即可設定應用裝置VM。按一下「下一步」
+
image:express-direct-attach-aff220-deploy_image67.png["vCenter 伺服器應用裝置安裝程式「設定應用裝置 VM」頁面的快照，包含 VM 名稱和 root 密碼欄位。"]

. 選擇最適合您環境的部署規模。按一下「下一步」
+
image:express-direct-attach-aff220-deploy_image68.png["vCenter 伺服器應用裝置安裝程式選擇部署大小頁面的快照，其中已選擇「微型」。"]

. 選取「infra_datastore_1」資料存放區。按一下「下一步」
+
image:express-direct-attach-aff220-deploy_image69.png["vCenter 伺服器設備安裝程式選擇資料存放區頁面的螢幕截圖，其中三個資料存放區中列出了 infra_datastore_1。"]
...
.. 輸入DNS伺服器。
+
image:express-direct-attach-aff220-deploy_image70.png["vCenter 伺服器設備安裝程式「設定網路設定」頁面的快照，包含靜態 IPv4 詳細資訊"]
...
+
image:express-direct-attach-aff220-deploy_image71.png["快照顯示第一階段 vCenter Server 部署成功，並帶有「繼續」按鈕。"]
...
. 按一下「確定」。
+
image:express-direct-attach-aff220-deploy_image72.png["已為 Express 叢集啟用 DRS 和 vSphere HA 的「新建叢集」對話框的快照"]
...
. 在叢集的「動作」功能表中選取「新增主機」。
+
image:express-direct-attach-aff220-deploy_image73.png["vSphere Client 操作選單的快照，顯示 Express 叢集已選取「新增主機」"]
