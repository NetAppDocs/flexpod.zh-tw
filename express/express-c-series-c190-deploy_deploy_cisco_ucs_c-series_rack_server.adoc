...
+
image:express-c-series-c190-deploy_image5.png["快照顯示 Cisco 伺服器開機畫面，提示使用者按 F8 進入 CIMC 設定。"]
...
+
image:express-c-series-c190-deploy_image6.png["Cisco IMC 組態公用程式 NIC 屬性的快照，其中已選擇專用 NIC 且未選擇備援。"]
...
. 按一下「設定開機順序」、然後按一下「確定」。
+
image:express-c-series-c190-deploy_image7.png["Cisco Integrated Management Controller BIOS 設定開機順序頁面的快照"]
...
. 按一下[儲存變更]，然後按一下[關閉]。
+
image:express-c-series-c190-deploy_image8.png["已啟用 KVM-MAPPED-DVD、iSCSI-A 和 iSCSI-B 的進階開機順序組態快照"]
...
. 如果該值尚未停用、請將其設為停用。
+
image:express-c-series-c190-deploy_image9.png["快照顯示 CIMC BIOS 設定，其中 PCIe Slot 1 和 Slot 2 的 Option ROM 已停用。"]
...
. 儲存變更並重新啟動CIMC。
+
image:express-c-series-c190-deploy_image10.png["Cisco Integrated Management Controller 介面卡 MLOM「一般」標籤的快照，其中「連接埠通道」未選取。"]
...
** 將上行鏈路連接埠設定為3。
+
image:express-c-series-c190-deploy_image11.png["快照顯示 VLAN 3439 和上行連接埠 1 的 iSCSI 虛擬網路卡 eth1 的一般設定。"]



. 選取左側的vNIC eth1。
+
image:express-c-series-c190-deploy_image12.png["快照顯示 vNIC eth1 的 iSCSI 開機屬性啟動器設定。"]
...
+
image:express-c-series-c190-deploy_image13.png["包含啟動器、主要目標和次要目標詳細資料的 iSCSI 開機屬性快照"]
...
+
image:express-c-series-c190-deploy_image14.png["快照顯示 vNIC eth3 的 iSCSI 開機屬性，包括啟動器和目標設定。"]
...
. 將VLAN設定為原生VLAN 2。
+
image:express-c-series-c190-deploy_image15.png["Cisco Integrated Management Controller vNIC eth0 屬性的快照，MTU 為 9000，預設 VLAN 為 2。"]

. 對eth1重複步驟3和4、確認上行鏈路連接埠對eth1設定為1。
+
image:express-c-series-c190-deploy_image16.png["Cisco Integrated Management Controller 虛擬網路卡 （vNIC） 的快照，列出 eth0 至 eth3 的 MTU、上行鏈路連接埠和 VLAN 設定"]
