...
. 按一下啟動KVM主控台。
+
image:express-c-series-aff220-deploy_image21.png["Cisco CIMC 伺服器 BIOS 頁面快照，頂部選單中含有「啟動 KVM」連結"]
...
=== 下載 ESXi 6.7 Cisco 自訂鏡像

. 導覽至 https://support.broadcom.com/group/ecx/downloads["Broadcom 支援入口網站下載網站"^]自訂 ISO 頁面。
...
+
image:express-c-series-aff220-deploy_image22.png["ESXi 安裝程式選擇容量為 15.00 GiB 的 NETAPP LUN C 模式儲存設備的螢幕截圖"]
...
+
image:express-c-series-aff220-deploy_image23.png["ESXi 網路介面卡選單的快照，其中 vmnic0 和 vmnic1 已為 vSwitch0 選取。"]
...
+
image:express-c-series-aff220-deploy_image24.png["ESXi 網路虛擬交換器標籤的快照，顯示了 vSwitch0 和 iScsiBootvSwitch"]
...
** 按一下「建立」。
+
image:express-c-series-aff220-deploy_image25.png["這是使用靜態 IPv4 設定和 MTU 9000 配置 iScsiBootPG-B 的「新增 VMkernel NIC」表單的快照。"]
...
+
image:express-c-series-aff220-deploy_image26.png["VMware ESXi 儲存設備介面卡頁面快照，其中已選擇 iSCSI 軟體介面卡 vmhba64。"]

. 按一下「動態目標」下的「新增動態目標」。
+
image:express-c-series-aff220-deploy_image27.png["Configure iSCSI 對話框的快照，其中「動態目標」下方新增了動態目標。"]
...
** 按一下儲存組態。
+
image:express-c-series-aff220-deploy_image28.png["快照顯示四個 iSCSI 動態目標 IP 位址，每個位址均使用連接埠 3260。"]
...
. 選取vSwitch0。
+
image:express-c-series-aff220-deploy_image29.png["VMware ESXi vSwitch0 頁面快照，顯示 MTU 為 1500，並且有兩個作用中的實體介面卡。"]
...
. 以滑鼠右鍵按一下「連接埠群組」索引標籤
+
image:express-c-series-aff220-deploy_image30.png["ESXi 網路頁面的快照，其中「連接埠群組」標籤已選取，「新增連接埠群組」可用。"]
...
+
image:express-c-series-aff220-deploy_image31.png["ESXi 網路 VMkernel NIC 索引標籤中列出 vmk0 和 vmk1 介面卡的快照"]
...
** 按一下「建立」。
+
image:express-c-series-aff220-deploy_image32.png["這是「新增 VMkernel NIC」表單的快照，顯示使用靜態 IPv4 設定配置 NFS 網路連接埠群組的畫面。"]
...
+
image:express-c-series-aff220-deploy_image33.png["具有靜態 IPv4 設定和已選取 vMotion 的「新增 VMkernel 網路卡」對話方塊快照"]
...
+
image:express-c-series-aff220-deploy_image34.png["VMware ESXi 儲存設備頁面快照，可見「新資料儲存」按鈕。"]

. 選取「掛載NFS資料存放區」。
+
image:express-c-series-aff220-deploy_image35.png["這是「新資料儲存」精靈的快照，其中已選擇「掛載 NFS 資料儲存」作為建立類型。"]
...
+
image:express-c-series-aff220-deploy_image36.png["ESXi 管理系統時間和日期頁面的快照，顯示 NTP 用戶端狀態和 NTP 伺服器"]
...
. 按一下儲存。
+
image:express-c-series-aff220-deploy_image37.png["啟用 NTP 並輸入 NTP 伺服器後，「編輯時間組態」對話方塊的快照。"]
...
+
image:express-c-series-aff220-deploy_image38.png["VMware ESXi 管理系統交換頁面快照，顯示交換設定。"]

. 按一下「編輯設定」。從資料存放區選項中選取infra_swap。
+
image:express-c-series-aff220-deploy_image39.png["快照顯示「編輯交換組態」對話框，其中已選擇 infra_swap 作為資料存放區，並顯示「儲存」和「取消」按鈕。"]
...
. 重新開機ESXi主機。
+
image:express-c-series-aff220-deploy_image40.png["終端顯示 NetApp NFS 插件 VIB 安裝成功，並提示需要重新開機。"]
