...
. 按一下啟動KVM主控台。
+
image:express-c-series-c190-deploy_image17.png["Cisco CIMC 伺服器 BIOS 頁面快照，頂部選單中含有「啟動 KVM」連結"]
...
+
image:express-c-series-c190-deploy_image18.png["ESXi 安裝程式選擇容量為 15.00 GiB 的 NETAPP LUN C 模式儲存設備的螢幕截圖"]
...
+
image:express-c-series-c190-deploy_image19.png["ESXi 網路介面卡畫面的螢幕截圖，其中已選擇 vmnic2 和 vmnic4 用於管理連網。"]
...
+
image:express-c-series-c190-deploy_image20.png["ESXi 網路頁面的快照，其中「虛擬交換器」標籤顯示了 vSwitch0 和 iScsiBootvSwitch"]
...
+
image:express-c-series-c190-deploy_image21.png["這是 iSCSIBootPG-A 組隊和故障轉移設定的快照，其中 vmnic3 作用中，vmnic5 未使用。"]
...
+
image:express-c-series-c190-deploy_image22.png["ESXi 儲存設備介面卡索引標籤的快照，其中已選取 iSCSI 軟體介面卡 vmhba64。"]

. 按一下「動態目標」下的「新增動態目標」。
+
image:express-c-series-c190-deploy_image23.png["快照顯示已設定四個 IP 位址和連接埠 3260 的 iSCSI 動態目標。"]
...
.. 按一下儲存組態。
+
image:express-c-series-c190-deploy_image24.png["快照顯示四個使用連接埠 3260 的 iSCSI 動態目標位址以及「儲存配置」按鈕。"]
...
. 選取vSwitch0。
+
image:express-c-series-c190-deploy_image25.png["ESXi vSwitch0 頁面快照，顯示 MTU 9000 及作用中的 vmnic2 和 vmnic4 介面卡"]
...
. 以滑鼠右鍵按一下「連接埠群組」索引標籤
+
image:express-c-series-c190-deploy_image26.png["ESXi 網路頁面的快照，其中「連接埠群組」標籤已選取，「新增連接埠群組」可用。"]
...
+
image:express-c-series-c190-deploy_image27.png["ESXi 網路 VMkernel NIC 標籤中列出 vmk0 和 vmk1 介面的畫面快照"]
...
+
image:express-c-series-c190-deploy_image28.png["具有靜態 IPv4 設定和已選取 vMotion 的「新增 VMkernel 網路卡」對話方塊快照"]
...
+
image:express-c-series-c190-deploy_image29.png["VMware ESXi 儲存設備頁面快照，可見「新資料儲存」按鈕。"]

. 選取「掛載NFS資料存放區」。
+
image:express-c-series-c190-deploy_image30.png["這是「新資料儲存」精靈的快照，其中已選擇「掛載 NFS 資料儲存」作為建立類型。"]
...
. 按一下儲存。
+
image:express-c-series-c190-deploy_image31.png["快照顯示 ESXi 時間與日期設定，NTP 服務正在執行，且有三部 NTP 伺服器。"]
...
+
image:express-c-series-c190-deploy_image32.png["VMware ESXi 管理系統交換頁面快照，顯示交換設定。"]

. 按一下「編輯設定」。從資料存放區選項中選取「infra_swap」。
+
image:express-c-series-c190-deploy_image33.png["快照顯示「編輯交換組態」對話框，其中已選擇 infra_swap 作為資料存放區，並顯示「儲存」和「取消」按鈕。"]
