...
image:flexpod-fips-image3.png["此圖展示加密寫入和解密讀取資料在網路、WAFL、RAID 和儲存設備中的流動情況，並包含一個加密模組。"]
...
image:flexpod-fips-image4.png["此圖展示了透過網路、WAFL、RAID 和儲存加密模組層的加密和解密過程。"]
...
image:flexpod-fips-image5.png["此圖展示了 ONTAP 加密邊界，包括金鑰管理程式、cryptomod 和 cryptomod_fips 元件。"]
