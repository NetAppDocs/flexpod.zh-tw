...
image:flexpod-genomics-image8.png["此圖展示從樣本採集到定序、變異分析、電子病歷整合和預測模型的基因組學工作流程。"]
...
+
image:flexpod-genomics-image10.png["VLAN IP 位址預留表，顯示位址範圍、子網路遮罩以及管理、虛擬機器、NFS 和 iSCSI 用途"]

. 在ONTAP SVM上設定iSCSI型開機LUN。
+
image:flexpod-genomics-image9.png["ONTAP System Manager 的螢幕擷取畫面，顯示 Healthcare_SVM 儲存設備 VM 中的六個 ESXi 開機 LUN。"]

. 將LUN對應至iSCSI啟動器群組。
+
image:flexpod-genomics-image11.png["快照顯示 ESXi_Boot_Lun_1 映射到 GenomicsESXi_1 iSCSI 啟動器群組"]

+
image:flexpod-genomics-image12.png["ONTAP System Manager 的快照，顯示映射到 GenomicsESXi_2 啟動器群組的 ESXi 開機 LUN 詳細資訊。"]

. 安裝vSphere 7.0搭配iSCSI開機。
. 向vCenter登錄ESXi主機。
+
image:flexpod-genomics-image13.png["vSphere Client 清單的快照，顯示包含 ESXi 主機和虛擬機器的 Genomics 資料夾。"]

. 將NFS資料存放區「infra_datastore_nfs」配置到ONTAP 整個儲存區。
+
image:flexpod-genomics-image14.png["ONTAP System Manager 的螢幕擷取畫面，顯示所選的 Healthcare_Workload_NFS_Vol NFS Volume 處於線上狀態"]

. 將資料存放區新增至vCenter。
+
image:flexpod-genomics-image16.png["vSphere Client 的快照，顯示 infra_datastore_nfs NFS 3 資料儲存區總結"]

. 使用vCenter將NFS資料存放區新增至ESXi主機。
+
image:flexpod-genomics-image15.png["vSphere Client 的快照，顯示 infra_datastore_nfs 主機已連接且狀態正常"]
...
+
image:flexpod-genomics-image17.png["Linux 終端機快照顯示 /dev/sdb 掛載在 /mnt/genomics 目錄下，使用率為 2%。"]
