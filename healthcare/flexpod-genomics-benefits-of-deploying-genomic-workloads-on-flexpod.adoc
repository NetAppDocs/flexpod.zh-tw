...
image:flexpod-genomics-image3.png["此圖顯示透過去重複處理減少的資料區塊，而中繼資料保持不變。"]
...
image:flexpod-genomics-image4.png["一張圖表，展示 FabricPool 如何在本地 SSD 和雲端物件儲存之間自動分層作用中資料和非活動資料。"]
...
image:flexpod-genomics-image5.png["顯示客戶、Cisco、NetApp 與提報團隊之間 FlexPod 協同支援互動的圖表"]
